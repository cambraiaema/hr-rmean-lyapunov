...
function hr_rede_model!(du, u, p, t)
    ...
    du[:, 1] = b.*(u[:, 1].^2) .- a .* (u[:, 1].^3) .+ u[:, 2] .- u[:, 3] .+ I .+ (γ/norm).*sum(u[:, 1])
    du[:, 2] = c .- d.*u[:, 1].^2 .- u[:, 2]
    du[:, 3] = r.*(s.*(u[:, 1] .+ k) .- u[:, 3])
...
function main()
    ...
    II = [ 4.405, 4.404, 4.403, 4.402, 4.401, 4.40, 4.399, 4.398, 4.397, 4.396]
    BB = [2.625, 2.626, 2.627, 2.628, 2.629, 2.63, 2.631, 2.632, 2.633, 2.634]
    ...
    Threads.@threads for i=1:length(II)
        ...
        for idx_γ=1:resolucao
            ...
            open("HR_parord_$(I)_$(b)_global.txt", "w") do io
